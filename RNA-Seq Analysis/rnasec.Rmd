...
```{r Download recount project data}
project <- "SRP038101"
...
**Please note the csv file with SRA metadata is already included with this Rmd, the following is only required if its desired to download the csv file from its original source**

A Python packages named SRAweb is required to download SRA data, this package is installed in a virtual environment in ./env subdirectory of the Rmd working directory.
...
get_sra_metadata <- function(project) {
  file_name <- glue("{project}_metadata.csv")
...
```{r Build DESeq object and plot dispersions}
dds <- DESeq(dds)
...
dupes <- duplicated(resdf_shrink_sig$SYMBOL) |
  duplicated(resdf_shrink_sig$SYMBOL, fromLast = TRUE)

kable(resdf_shrink_sig[dupes, ], caption = "Duplicated gene symbols")
```


## Duplicated symbols

The gene symbols in the table above contains duplicated SYMBOLS, only the one with lowest padj will be kept when building GSEA rankings
...
ev
```
The Volcano Plot shows only a few (n = `r nrow(ev$data[ev$data$Sig == "FC_P" & ev$data$log2FoldChange > 0,])`) genes in above the FC and P value thresholds in the over-expression side, and `r nrow(ev$data[ev$data$Sig == "FC_P" & ev$data$log2FoldChange < 0,])` in the under-expression side.
...
```
An histogram of GSEA metric is presented, showing a normal distribution, with a mild skew on the > 0 side.

By dropping rows where padj = NA, the ties in the preranked stats is reduced from ~12% to ~2%

```{r Build ranks df and run GSEA, warning=FALSE}
ranks <- resdf2 %>%
...
datatable(gsearesdf2_down, caption = "Suppressed pathways")
```
...
datatable(gsearesdf2_up, caption = "Activated pathways")
```
