...
```{r}
df = as_tibble(read.csv("gapminder_clean.csv", check.names=F, stringsAsFactors=T)[,-1]) %>% 
        mutate_all(~na_if(.,"")) %>%
...
data.frame(`Unique values`=sapply(lapply(df, unique), length), `NA count`=colSums(is.na(df)), Type=sapply(df, class), check.names=F)
...
#conts <- sapply(df, function(x) !is.factor(x))

df %>% dplyr::select(where(is.numeric)) %>% gather() %>% filter(!is.na(value))  %>%
  ggplot(aes(value)) + 
...
df_continent = df %>% 
                ...
                dplyr::select(continent,`Energy use (kg of oil equivalent per capita)`, pop, `Country Name`)
dim(df_continent)
...
p = df_continent %>% 
      ...
      geom_boxplot(aes(col=continent)) + labs(title="Energy use vs continent", x="GPD Per Capita") + scale_y_log10()
...
df_1990 = df %>% 
            filter(as.numeric(as.character(Year)) > 1990 & continent %in% c("Europe","Asia") & !is.na(`Imports of goods and services (% of GDP)`)) %>% 
...
p = df_1990 %>% 
      ggplot(aes(x=continent, y=`Imports of goods and services (% of GDP)`, col=continent)) + geom_boxplot()
